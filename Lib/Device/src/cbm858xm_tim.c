/**
* @file        cbm858xm_tim.c
* @brief       This file provides all the basic TIM firmware functions.
...
void tim_base_init(tim_reg_t* ptr_timer, tim_base_t* ptr_base)
{
    ...
    if ((ptr_timer == TIM1) || (ptr_timer == TIM3))
    {
        /* Select the Counter  */
...
void tim_choc_init(tim_reg_t* ptr_timer, tim_choc_t* ptr_choc)
{
    ...
    MODIFY_REG(ptr_timer->CHxCFGR[ch >> 1], (TIM_CH12CFGR_CH1FS|TIM_CH12CFGR_CH1OCMSEL) << ((ch & 0x1) * 8), \
               ptr_choc->mode << ((ch & 0x1) * 8));

    /* Set the Output Compare Polarity and Output State */
    MODIFY_REG(ptr_timer->CCCTR, (TIM_CCCTR_CH1CCP|TIM_CCCTR_CH1CCEN) << (ch * 4), \
               (ptr_choc->polarity | ptr_choc->output_state) << (ch * 4));

    if(ptr_timer == TIM1)
    { 
        /* Set the Output N Polarity and Output N State */
        MODIFY_REG(ptr_timer->CCCTR, (TIM_CCCTR_CH1NCCP|TIM_CCCTR_CH1NCCEN) << (ch * 4), \
                   (ptr_choc->polarity_n | ptr_choc->output_state_n) << (ch * 4));		

        /* Set the Output Idle state and Output N Idle state */
        MODIFY_REG(ptr_timer->CTR2, (TIM_CTR2_IVO1|TIM_CTR2_IVO1N) << (ch * 2), \
                   (ptr_choc->idle_state | ptr_choc->idle_state_n) << (ch * 2));
    }
...
void tim_choc_preload_set(tim_reg_t* ptr_timer, uint8_t ch, uint16_t value)
{
    ...
    /* Enable or disable channel 1~4 output compare preload value */
...
void tim_choc_fast_set(tim_reg_t* ptr_timer, uint8_t ch, uint16_t value)
{
    ...
    /* Enable or disable channel 1~4 output compare fast feature */
...
void tim_ti_external_clock_config(tim_reg_t* ptr_timer, uint16_t value, uint16_t polarity, uint16_t filter)                                
{
    ...
    if (value == TIM_TIMx_EXTICLK1_SOURCE_TI2)
    {
...
void tim_encoder_interface_set(tim_reg_t* ptr_timer, uint16_t mode, uint16_t polarity, uint16_t value)                               
{
    ...
    MODIFY_REG(ptr_timer->CCCTR , TIM_CCCTR_CH1CCP|TIM_CCCTR_CH1NCCP|TIM_CCCTR_CH2CCP|TIM_CCCTR_CH2NCCP, \
               polarity | (value << 4));
}
...
/**
  ...
  * @param  ptr_timer: pointer to TIMx where x can be 1, 3 or 14 .  
  * @param  ch: channel to TIM1 and TIM3 can be 1,2,3,4, the TIM14 only can be 1.
...
static void tim_chic_config(tim_reg_t* ptr_timer, uint8_t ch, uint16_t polarity, uint16_t select, uint16_t filter)                       
{
    ...
    MODIFY_REG(ptr_timer->CHxCFGR[idx], (TIM_CH12CFGR_CH1FS|TIM_CH12CFGR_CH1ICFLT) << (shift * 8), \
               (select | (filter << 4)) << (shift * 8));

    /* Select the Polarity and set the CH1CCEN bit */	
    MODIFY_REG(ptr_timer->CCCTR, (TIM_CCCTR_CH1CCP|TIM_CCCTR_CH1NCCP) << (ch * 4), \
               (polarity | TIM_CCCTR_CH1CCEN) << (ch * 4));
}
